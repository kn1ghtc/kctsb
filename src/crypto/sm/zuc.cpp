/**
 * @file zuc.cpp
 * @brief ZUC Stream Cipher Implementation - GB/T 33133
 *
 * ZUC is a Chinese stream cipher for 4G LTE mobile communication security.
 * This implementation provides key stream generation, confidentiality (EEA3)
 * and integrity (EIA3) algorithms.
 *
 * C++ Core + C ABI Architecture (v3.4.0)
 *
 * @author knightc
 * @copyright Copyright (c) 2019-2026 knightc. All rights reserved.
 * @license Apache-2.0
 */

#include "kctsb/crypto/sm/zuc.h"
#include "kctsb/core/common.h"
#include <cstdlib>
#include <cstring>

namespace kctsb {
namespace internal {

// ============================================================================
// ZUC S-boxes and D table
// ============================================================================

// S0 S-box (256 bytes)
static const uint8_t ZUC_S0[256] = {
    
};

// S1 S-box (256 bytes)
static const uint8_t ZUC_S1[256] = {
    
};

// D constant table (16 elements)
static const uint32_t ZUC_D[16] = {
    0x44D7, 0x26BC, 0x626B, 0x135E,
    0x5789, 0x35E2, 0x7135, 0x09AF,
    0x4D78, 0x2F13, 0x6BC4, 0x1AF1,
    0x5E26, 0x3C4D, 0x789A, 0x47AC
};

// ============================================================================
// Helper Macros and Functions
// ============================================================================

#define ROTL32(x, n) (((x) << (n)) | ((x) >> (32 - (n))))

// Link key, d, iv to generate s-register value
static inline uint32_t link_to_s(uint8_t k, uint16_t d, uint8_t iv) {
    return (static_cast<uint32_t>(k) << 23) | (static_cast<uint32_t>(d) << 8) | static_cast<uint32_t>(iv);
}

// a + b mod (2^31 - 1)
static inline uint32_t add_mod(uint32_t a, uint32_t b) {
    uint32_t c = a + b;
    if (c >> 31) {
        c = (c & 0x7FFFFFFF) + 1;
    }
    return c;
}

// x * 2^k mod (2^31 - 1)
static inline uint32_t pow_mod(uint32_t x, uint32_t k) {
    return (((x << k) | (x >> (31 - k))) & 0x7FFFFFFF);
}

// Linear transformation L1
static inline uint32_t L1(uint32_t X) {
    return X ^ ROTL32(X, 2) ^ ROTL32(X, 10) ^ ROTL32(X, 18) ^ ROTL32(X, 24);
}

// Linear transformation L2
static inline uint32_t L2(uint32_t X) {
    return X ^ ROTL32(X, 8) ^ ROTL32(X, 14) ^ ROTL32(X, 22) ^ ROTL32(X, 30);
}

// ============================================================================
// ZUC Internal State Class
// ============================================================================

class ZUCEngine {
public:
    uint32_t lfsr_s[16];    // LFSR state
    uint32_t br_x[4];       // Bit reconstruction output
    uint32_t f_r[2];        // Non-linear function state (R1, R2)

    void init(const uint8_t key[16], const uint8_t iv[16]) {
        // Load key to LFSR
        for (size_t i = 0; i < 16; i++) {
            lfsr_s[i] = link_to_s(key[i], static_cast<uint16_t>(ZUC_D[i]), iv[i]);
        }

        // Initialize R1, R2
        f_r[0] = 0;
        f_r[1] = 0;

        // 32 rounds of initialization
        for (int count = 0; count < 32; count++) {
            bit_reconstruction();
            uint32_t w = nonlinear_f();
            lfsr_init_mode(w >> 1);
        }
    }

    uint32_t generate() {
        bit_reconstruction();
        uint32_t z = nonlinear_f() ^ br_x[3];
        lfsr_work_mode();
        return z;
    }

    void generate_keystream(uint32_t* keystream, int len) {
        // Discard first output
        bit_reconstruction();
        nonlinear_f();
        lfsr_work_mode();

        // Generate keystream
        for (int i = 0; i < len; i++) {
            keystream[i] = generate();
        }
    }

private:
    void bit_reconstruction() {
        br_x[0] = ((lfsr_s[15] & 0x7FFF8000) << 1) | (lfsr_s[14] & 0x0000FFFF);
        br_x[1] = ((lfsr_s[11] & 0x0000FFFF) << 16) | ((lfsr_s[9] & 0x7FFF8000) >> 15);
        br_x[2] = ((lfsr_s[7] & 0x0000FFFF) << 16) | ((lfsr_s[5] & 0x7FFF8000) >> 15);
        br_x[3] = ((lfsr_s[2] & 0x0000FFFF) << 16) | ((lfsr_s[0] & 0x7FFF8000) >> 15);
    }

    uint32_t nonlinear_f() {
        uint32_t w = (br_x[0] ^ f_r[0]) + f_r[1];
        uint32_t w1 = f_r[0] + br_x[1];
        uint32_t w2 = f_r[1] ^ br_x[2];

        // Update R1
        f_r[0] = L1((w1 << 16) | (w2 >> 16));
        f_r[0] = (ZUC_S0[(f_r[0] >> 24) & 0xFF] << 24) |
                 (ZUC_S1[(f_r[0] >> 16) & 0xFF] << 16) |
                 (ZUC_S0[(f_r[0] >> 8) & 0xFF] << 8) |
                 ZUC_S1[f_r[0] & 0xFF];

        // Update R2
        f_r[1] = L2((w2 << 16) | (w1 >> 16));
        f_r[1] = (ZUC_S0[(f_r[1] >> 24) & 0xFF] << 24) |
                 (ZUC_S1[(f_r[1] >> 16) & 0xFF] << 16) |
                 (ZUC_S0[(f_r[1] >> 8) & 0xFF] << 8) |
                 ZUC_S1[f_r[1] & 0xFF];

        return w;
    }

    void lfsr_init_mode(uint32_t u) {
        uint32_t v = lfsr_s[0];
        v = add_mod(v, pow_mod(lfsr_s[15], 15));
        v = add_mod(v, pow_mod(lfsr_s[13], 17));
        v = add_mod(v, pow_mod(lfsr_s[10], 21));
        v = add_mod(v, pow_mod(lfsr_s[4], 20));
        v = add_mod(v, pow_mod(lfsr_s[0], 8));

        // Shift LFSR
        for (int i = 0; i < 15; i++) {
            lfsr_s[i] = lfsr_s[i + 1];
        }

        lfsr_s[15] = add_mod(v, u);
        if (lfsr_s[15] == 0) {
            lfsr_s[15] = 0x7FFFFFFF;
        }
    }

    void lfsr_work_mode() {
        uint32_t v = lfsr_s[0];
        v = add_mod(v, pow_mod(lfsr_s[15], 15));
        v = add_mod(v, pow_mod(lfsr_s[13], 17));
        v = add_mod(v, pow_mod(lfsr_s[10], 21));
        v = add_mod(v, pow_mod(lfsr_s[4], 20));
        v = add_mod(v, pow_mod(lfsr_s[0], 8));

        // Shift LFSR
        for (int i = 0; i < 15; i++) {
            lfsr_s[i] = lfsr_s[i + 1];
        }

        lfsr_s[15] = v;
        if (lfsr_s[15] == 0) {
            lfsr_s[15] = 0x7FFFFFFF;
        }
    }
};

// Bit value helper for integrity algorithm
static inline uint8_t bit_value(const uint32_t* M, uint32_t i) {
    uint32_t j = i >> 5;
    uint32_t k = i & 0x1F;
    return (M[j] & (0x1U << (31 - k))) ? 1 : 0;
}

// Get 32-bit word from bit position
static inline uint32_t get_word(const uint32_t* k, uint32_t i) {
    uint32_t j = i >> 5;
    uint32_t m = i & 0x1F;
    if (m == 0) {
        return k[j];
    }
    return (k[j] << m) | (k[j + 1] >> (32 - m));
}

}  // namespace internal
}  // namespace kctsb

// ============================================================================
// C ABI Exports
// ============================================================================

extern "C" {

void kctsb_zuc_init(kctsb_zuc_ctx_t* ctx, const uint8_t key[16], const uint8_t iv[16]) {
    if (!ctx || !key || !iv) return;

    auto* engine = new kctsb::internal::ZUCEngine();
    engine->init(key, iv);
    ctx->internal = engine;
}

void kctsb_zuc_generate(kctsb_zuc_ctx_t* ctx, uint32_t* keystream, size_t len) {
    if (!ctx || !ctx->internal || !keystream || len == 0) return;

    auto* engine = static_cast<kctsb::internal::ZUCEngine*>(ctx->internal);

    // Use the public generate_keystream method
    engine->generate_keystream(keystream, static_cast<int>(len));
}

void kctsb_zuc_free(kctsb_zuc_ctx_t* ctx) {
    if (!ctx || !ctx->internal) return;

    auto* engine = static_cast<kctsb::internal::ZUCEngine*>(ctx->internal);
    delete engine;
    ctx->internal = nullptr;
}

void kctsb_zuc_eea3(const uint8_t ck[16], uint32_t count, uint8_t bearer,
                    uint8_t direction, const uint32_t* ibs, int length, uint32_t* obs) {
    if (!ck || !ibs || !obs || length <= 0) return;

    // Generate IV from count, bearer, direction
    uint8_t iv[16];
    iv[0] = static_cast<uint8_t>(count >> 24);
    iv[1] = static_cast<uint8_t>((count >> 16) & 0xFF);
    iv[2] = static_cast<uint8_t>((count >> 8) & 0xFF);
    iv[3] = static_cast<uint8_t>(count & 0xFF);
    iv[4] = ((bearer << 3) | (direction << 2)) & 0xFC;
    iv[5] = 0x00;
    iv[6] = 0x00;
    iv[7] = 0x00;
    iv[8] = iv[0];
    iv[9] = iv[1];
    iv[10] = iv[2];
    iv[11] = iv[3];
    iv[12] = iv[4];
    iv[13] = iv[5];
    iv[14] = iv[6];
    iv[15] = iv[7];

    // Generate keystream length
    size_t L = static_cast<size_t>((length + 31) / 32);
    uint32_t* k = static_cast<uint32_t*>(std::malloc(sizeof(uint32_t) * L));
    if (!k) return;

    // Initialize and generate keystream
    kctsb::internal::ZUCEngine engine;
    engine.init(ck, iv);
    engine.generate_keystream(k, static_cast<int>(L));

    // XOR to produce output
    for (size_t i = 0; i < L; i++) {
        obs[i] = ibs[i] ^ k[i];
    }

    // Mask last word
    int t = length % 32;
    if (t) {
        obs[L - 1] = (obs[L - 1] >> (32 - t)) << (32 - t);
    }

    std::free(k);
}

uint32_t kctsb_zuc_eia3(const uint8_t ik[16], uint32_t count, uint8_t bearer,
                        uint8_t direction, const uint32_t* message, int length) {
    if (!ik || !message || length <= 0) return 0;

    // Generate IV from count, bearer, direction
    uint8_t iv[16];
    iv[0] = static_cast<uint8_t>(count >> 24);
    iv[1] = static_cast<uint8_t>((count >> 16) & 0xFF);
    iv[2] = static_cast<uint8_t>((count >> 8) & 0xFF);
    iv[3] = static_cast<uint8_t>(count & 0xFF);
    iv[4] = bearer << 3;
    iv[5] = 0x00;
    iv[6] = 0x00;
    iv[7] = 0x00;
    iv[8] = iv[0] ^ (direction << 7);
    iv[9] = iv[1];
    iv[10] = iv[2];
    iv[11] = iv[3];
    iv[12] = iv[4];
    iv[13] = iv[5];
    iv[14] = iv[6] ^ (direction << 7);
    iv[15] = iv[7];

    // Generate keystream length
    size_t L = static_cast<size_t>((length + 31) / 32 + 2);
    uint32_t* k = static_cast<uint32_t*>(std::malloc(sizeof(uint32_t) * L));
    if (!k) return 0;

    // Initialize and generate keystream
    kctsb::internal::ZUCEngine engine;
    engine.init(ik, iv);
    engine.generate_keystream(k, static_cast<int>(L));

    // Compute MAC
    uint32_t T = 0;
    for (uint32_t i = 0; i < static_cast<uint32_t>(length); ++i) {
        if (kctsb::internal::bit_value(message, i)) {
            T ^= kctsb::internal::get_word(k, i);
        }
    }

    // T = T ^ k_LENGTH
    T ^= kctsb::internal::get_word(k, static_cast<uint32_t>(length));

    // MAC = T ^ k(32*(L-1))
    uint32_t MAC = T ^ kctsb::internal::get_word(k, static_cast<uint32_t>(32 * (L - 1)));

    std::free(k);
    return MAC;
}

}  // extern "C"
