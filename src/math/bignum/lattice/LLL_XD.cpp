               ...
               xdouble *c, long k, xdouble bound, long st, xdouble *buf)
{
   ...
   for (j = st; j <= k-1; j++) {
      if (b[k]*b[j] < KCTSB_FDOUBLE_PRECISION*KCTSB_FDOUBLE_PRECISION) {
         double z = 0;
         xdouble *B1_k = B1[k];
         xdouble *B1_j = B1[j];
           ...
           long m, long init_k, long &quit)
{
   ...
   KCTSB_TLS_LOCAL_INIT(xdouble, bound, (to_xdouble(0)));
...
KCTSB_TLS_GLOBAL_DECL(vec_xdouble, BKZConstant)
...
KCTSB_TLS_GLOBAL_DECL(vec_xdouble, BKZThresh)
