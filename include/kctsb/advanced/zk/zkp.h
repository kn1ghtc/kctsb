...
struct ProvingKey {
    ...
    G1Point beta_g1;
    G1Point delta_g1;
    std::vector<G1Point> a_query;   // [A_i(而)]??
    std::vector<G1Point> b_g1_query; // [B_i(而)]??
    std::vector<G1Point> h_query;    // [而??* t(而) / 汛]??
    std::vector<G1Point> l_query;    // [(汕*U_i(而) + 汐*V_i(而) + W_i(而)) / 汛]??
    
    // G2 elements
    G2Point beta_g2;
    G2Point delta_g2;
    std::vector<G2Point> b_g2_query; // [B_i(而)]??
...
struct VerificationKey {
    ...
    G2Point gamma_g2;
    G2Point delta_g2;
    std::vector<G1Point> ic;  // [(汕*U_i(而) + 汐*V_i(而) + W_i(而)) / 污]??for public inputs
...
public:
     ...
     * 
     * WARNING: The toxic waste (而, 汐, 汕, 污, 汛) must be securely discarded.
