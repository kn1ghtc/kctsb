...
/****************************************************************
    ...
    KCTSB v4.0.1: Windows LLP64 Compatibility Note
    ===============================================
    On Windows x64, sizeof(long) = 4 bytes (32-bit), but the algorithms
    in this file require 64-bit word operations.
    
    For Windows LLP64 compatibility, we define sp_word_t as the
    actual 64-bit type used for single-precision modular arithmetic.
...
// Windows LLP64 compatibility: define 64-bit word type
#include <cstdint>

#if defined(_WIN32) || defined(_WIN64)
    // Windows LLP64: long is 32-bit, need long long for 64-bit operations
    typedef long long sp_word_t;
    typedef unsigned long long sp_uword_t;
    #define SP_WORD_C(x) x##LL
    #define SP_UWORD_C(x) x##ULL
#else
    // Unix LP64: long is already 64-bit
    typedef long sp_word_t;
    typedef unsigned long sp_uword_t;
    #define SP_WORD_C(x) x##L
    #define SP_UWORD_C(x) x##UL
#endif
