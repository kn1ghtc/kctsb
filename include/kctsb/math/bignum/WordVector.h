...
#ifndef KCTSB_WordVector__H
#define KCTSB_WordVector__H

/**************************************************************

  A WordVector is functionally similar to
  a  generic bignum vector of _kctsb_ulong.  
...
#define KCTSB_WV_RANGE_CHECK_CODE 
#else
#define KCTSB_WV_RANGE_CHECK_CODE if (i < 0 || !rep || i >= long(rep[-1])) LogicError("index out of range in WordVector");
#endif

// vectors are allocated in chunks of this size

#ifndef KCTSB_WordVectorMinAlloc
#define KCTSB_WordVectorMinAlloc (4)
#endif

// controls initialization during input

#ifndef KCTSB_WordVectorInputBlock
#define KCTSB_WordVectorInputBlock 50
#endif
...
public:  
   ...
   {
      _kctsb_swap(this->rep, other.rep);
   }
   ...
   void KillBig() { if (MaxLength() > KCTSB_RELEASE_THRESH) kill(); }
   ...
   void QuickSetLength(long n) { rep[-1] = _kctsb_ulong(n); } 
  
   long length() const { return (!rep) ?  0 : long(rep[-1]); }  
   long MaxLength() const 
   { return (!rep) ?  0 : long(rep[-2] >> 1); } 
  
   _kctsb_ulong& operator[](long i)   
   {  
      KCTSB_WV_RANGE_CHECK_CODE  
      return rep[i];  
   }  
  
   const _kctsb_ulong& operator[](long i) const 
   {  
      KCTSB_WV_RANGE_CHECK_CODE  
      return rep[i];  
   }  
  
   _kctsb_ulong& operator()(long i) { return (*this)[i-1]; }  
   const _kctsb_ulong& operator()(long i) const { return (*this)[i-1]; } 
   
  
   const _kctsb_ulong* elts() const { return rep; }  
   _kctsb_ulong* elts() { return rep; }  
         
   void swap(WordVector& y);  
   void append(_kctsb_ulong a); 
...
inline void append(WordVector& v, _kctsb_ulong a)
   { v.append(a); }
...
KCTSB_SNS istream& operator>>(KCTSB_SNS istream&, WordVector&);  
KCTSB_SNS ostream& operator<<(KCTSB_SNS ostream&, const WordVector&);  
