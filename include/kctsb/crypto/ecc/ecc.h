/**
 * @file ecc.h
 * @brief Self-Contained Elliptic Curve Cryptography
 * 
...
struct AffinePoint {
    ...
    AffinePoint(const Fe256& x_, const Fe256& y_)
        : x(x_), y(y_), is_infinity(false) {}
    
    /**
     * @brief Create point from hex coordinates
     */
    AffinePoint(const char* hex_x, const char* hex_y);
     ...
     */
    bool ct_equal(const AffinePoint& other) const {
    ...
    bool operator==(const AffinePoint& other) const { return ct_equal(other); }
    bool operator!=(const AffinePoint& other) const { return !ct_equal(other); }
};
...
struct JacobianPoint {
     ...
     */
    JacobianPoint(const Fe256& x, const Fe256& y, const Fe256& z)
        : X(x), Y(y), Z(z) {}
     ...
     * @return Affine point
     */
    AffinePoint to_affine(const Fe256MontContext& ctx) const;
    
    /**
     * @brief Constant-time conditional move
     */
    void ct_cmov(const JacobianPoint& src, bool cond) {
     ...
     */
    static void ct_cswap(JacobianPoint& a, JacobianPoint& b, bool cond) {
...
public:
    ...
    const AffinePoint generator() const { return params_.generator(); }
    const std::string& name() const { return params_.name; }
    CurveType type() const { return params_.type; }
     ...
     * @param p Input point
     */
    void point_double(JacobianPoint* r, const JacobianPoint* p) const;
     ...
     */
    void point_add(JacobianPoint* r, 
                   const JacobianPoint* p, 
                   const JacobianPoint* q) const;
     ...
     */
    void point_add_mixed(JacobianPoint* r,
                         const JacobianPoint* p,
     ...
     */
    void point_negate(JacobianPoint* r, const JacobianPoint* p) const;
     ...
     * @param p Base point
     */
    void scalar_mul(JacobianPoint* r, 
                    const Fe256* k, 
                    const JacobianPoint* p) const;
     ...
     * @param k Scalar
     */
    void scalar_mul_base(JacobianPoint* r, const Fe256* k) const;
     ...
     */
    void scalar_mul_double(JacobianPoint* r,
                           const Fe256* k1,
                           const Fe256* k2,
                           const JacobianPoint* p) const;
...
private:
    CurveParams params_;
    JacobianPoint G_jac_;  ///< Generator in Jacobian form
};
...
struct ECDSASignature {
     ...
     */
    static ECDSASignature from_raw(const uint8_t* data);
};
...
public:
     ...
     */
    explicit ECDSA(CurveType curve = CurveType::SECP256K1);
...
public:
     ...
     */
    explicit ECDH(CurveType curve = CurveType::SECP256K1);
