...
/**
 * @brief Wide multiplication: r = a 脳 b (512-bit result)
 */
...
void fe256_reduce_sm2(fe256* r, const fe512* a) {
    ...
    // h[i] * 2^(256+64i) 鈮?h[i] * 2^(64i) * k (mod p)
    ...
    // 2^288 = 2^32 * 2^256 鈮?2^32 * k = 2^32 + 2^128 - 2^96 + 2^256 (recursive)
    ...
    for (int round = 0; round < 4; round++) {
        ...
        if (acc[4] != 0) {
            ...
            // overflow * 2^256 鈮?overflow * k (mod p)
            // k = 1 + 2^96 - 2^64 + 2^224
            acc[0] += overflow;
