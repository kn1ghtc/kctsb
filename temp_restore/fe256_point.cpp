...
void fe256_point_add(fe256_point* r, const fe256_point* p,
    ...
    // Z3 = ((Z1+Z2)虏 - Z1Z1 - Z2Z2) * H (using cached Z1, Z2)
    ops->add(&t0, &Z1, &Z2);
    ops->sqr(&t1, &t0);
    ops->sub(&t0, &t1, &Z1Z1);
    ops->sub(&t1, &t0, &Z2Z2);
...
void fe256_point_double(fe256_point* r, const fe256_point* p, int curve_type) {
    ...
    if (ops->a_is_zero) {
        ...
        // For a = -3: alpha = 3*X1虏 - 3*Z1鈦?= 3*(X1虏 - Z1鈦? = 3*(X1-Z1虏)(X1+Z1虏)
        ops->sub(&t0, &X1, &delta);  // X1 - Z1虏
        ops->add(&t1, &X1, &delta);  // X1 + Z1虏
        ops->mul(&A, &t0, &t1);        // (X1-Z1虏)(X1+Z1虏)
        ops->add(&t0, &A, &A);         // 2*...
        ...
        // Z3 = (Y1 + Z1)虏 - gamma - delta (using cached Y1, Z1)
        ops->add(&t0, &Y1, &Z1);
        ops->sqr(&t1, &t0);
        ops->sub(&t0, &t1, &gamma);
        ops->sub(&r->Z, &t0, &delta);

        // Y3 = alpha*(4*beta - X3) - 8*gamma虏
        ops->add(&t0, &beta, &beta);   // 2*beta
        ops->add(&t0, &t0, &t0);       // 4*beta
        ops->sub(&t1, &t0, &r->X);     // 4*beta - X3
        ops->mul(&t0, &alpha, &t1);    // alpha*(4*beta - X3)
        ops->sqr(&t1, &gamma);         // gamma虏
        ops->add(&t1, &t1, &t1);       // 2*gamma虏
        ops->add(&t1, &t1, &t1);       // 4*gamma虏
        ops->add(&t1, &t1, &t1);       // 8*gamma虏
        ops->sub(&r->Y, &t0, &t1);
