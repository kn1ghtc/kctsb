# kctsb v3.4.0 重构计划

> **版本**: 3.4.0
> **创建时间**: 2026-01-15 (Beijing Time, UTC+8)
> **状态**: 进行中

---

## 📋 重构目标概述

将 kctsb 项目完全重构为 **C++ 实现 + C ABI 封装** 的架构，追求极致性能和最小内存占用。

### 核心设计原则

| 优先级 | 原则 | 描述 |
|--------|------|------|
| **P1** | C++ Core + C ABI | C++17 实现核心逻辑，extern "C" 导出稳定 ABI |
| **P2** | C++17 统一标准 | 全项目使用 C++17，启用最优编译参数 |
| **P3** | 单文件单算法 | 每个算法一个 .cpp + 一个 .h，禁止额外封装层 |

---

## 🎯 重构范围

### 1. 目录结构调整

```
kctsb/
├── include/kctsb/crypto/
│   ├── aes.h              # AES-GCM (合并)
│   ├── chacha20.h         # ChaCha20-Poly1305 (合并)
│   ├── sha256.h           # SHA-256
│   ├── sha512.h           # SHA-512
│   ├── sha3.h             # SHA3-256/512 (Keccak)
│   ├── blake2.h           # BLAKE2b/s
│   ├── blake3.h           # BLAKE3
│   ├── sm2.h              # SM2 (合并 sm2_enc.h)
│   ├── sm3.h              # SM3 (删除 sm3_core.h)
│   ├── sm4.h              # SM4 (删除 sm4_core.hpp/sm4_impl.hpp)
│   ├── rsa.h              # RSA
│   ├── ecc.h              # ECC/ECDSA/ECDH/ECIES
│   └── ...
├── src/crypto/
│   ├── aes.cpp            # AES C++实现 + C ABI
│   ├── chacha20.cpp       # ChaCha20 C++实现 + C ABI
│   ├── sha256.cpp         # SHA-256 C++实现 + C ABI
│   ├── sha512.cpp         # SHA-512 C++实现 + C ABI
│   ├── sha3.cpp           # SHA3 C++实现 + C ABI
│   ├── blake2.cpp         # BLAKE2 C++实现 + C ABI
│   ├── blake3.cpp         # BLAKE3 C++实现 + C ABI
│   ├── sm2.cpp            # SM2 C++实现 + C ABI
│   ├── sm3.cpp            # SM3 C++实现 + C ABI
│   ├── sm4.cpp            # SM4 C++实现 + C ABI
│   ├── rsa.cpp            # RSA C++实现 + C ABI
│   ├── ecc.cpp            # ECC C++实现 + C ABI (合并ecdh/ecdsa/ecies)
│   └── ...
```

### 2. 需要删除的文件

```
# SM 系列冗余文件
- include/kctsb/crypto/sm/sm2_enc.h
- include/kctsb/crypto/sm/sm3_core.h
- include/kctsb/crypto/sm/sm3_impl.h
- include/kctsb/crypto/sm/sm4_core.hpp
- include/kctsb/crypto/sm/sm4_impl.hpp
- include/kctsb/crypto/sm/sm_util.h
- src/crypto/sm/sm_api.cpp        # 关键: 删除这个不合理的API封装
- src/crypto/sm/sm3.c             # 合并到 sm3.cpp
- src/crypto/sm/sm4.cpp           # 合并重构
- src/crypto/sm/sm_util.c

# Hash 系列需要合并的文件
- src/crypto/hash/Keccak.cpp      # 合并到 sha3.cpp
- src/crypto/hash/blake2.c        # 合并到 blake2.cpp
- src/crypto/hash/chacha20.c      # 合并到 ../chacha20.cpp
- src/crypto/hash/sha256.cpp      # 合并到 ../sha256.cpp
- src/crypto/hash/sha512.c        # 合并到 sha512.cpp
- src/crypto/hash/cmac.cpp        # 合并到 mac.cpp
- src/crypto/hash/gmac.cpp        # 合并到 mac.cpp
- src/crypto/hash/hmac.cpp        # 合并到 mac.cpp

# 冗余头文件
- include/kctsb/crypto/hash/blake2_impl.h
- include/kctsb/crypto/hash/chacha20_impl.h
- include/kctsb/crypto/hash/keccak.h  # 合并到 sha3.h
```

### 3. 需要重构的模块

| 模块 | 当前状态 | 重构目标 | 优先级 |
|------|----------|----------|--------|
| SHA-256 | sha256.cpp + sha.h | sha256.cpp + sha256.h | P1 |
| SHA-512 | sha512.c + sha.h | sha512.cpp + sha512.h | P1 |
| SHA3 | Keccak.cpp + keccak.h | sha3.cpp + sha3.h | P1 |
| BLAKE2 | blake2.c + blake.h | blake2.cpp + blake2.h | P1 |
| SM3 | sm3.c + sm3.h + sm3_core.h | sm3.cpp + sm3.h | P1 |
| SM4 | sm4.cpp + sm_api.cpp + 多头文件 | sm4.cpp + sm4.h | P1 |
| SM2 | 分散 + sm_api.cpp | sm2.cpp + sm2.h | P1 |
| AES | aes.cpp + aes.h | aes.cpp + aes.h (重构) | P2 |
| ChaCha20 | chacha20_poly1305.cpp | chacha20.cpp + chacha20.h | P2 |
| RSA | rsa.cpp + rsa.h | rsa.cpp + rsa.h (重构) | P2 |
| ECC | ecc_curve + ecdh + ecdsa + ecies | ecc.cpp + ecc.h | P2 |

---

## 📐 代码架构规范

### C++ Core + C ABI 模板

```cpp
/**
 * @file algorithm.cpp
 * @brief Algorithm - C++ Implementation + C ABI Export
 */

#include "kctsb/crypto/algorithm.h"
#include <array>
#include <cstdint>

// ============================================================================
// C++ Internal Implementation
// ============================================================================
namespace kctsb::internal {

class Algorithm {
public:
    // Template metaprogramming for compile-time optimization
    template<size_t N>
    static constexpr std::array<uint32_t, N> compute_constants() noexcept;

    // Force inline for hot path
    __attribute__((always_inline))
    void process_block(const uint8_t* input, uint8_t* output) noexcept;

    // Zero-copy in-place operation
    void transform_inplace(uint8_t* buffer, size_t len) noexcept;

private:
    // Aligned memory for SIMD
    alignas(32) std::array<uint32_t, 16> state_;
};

} // namespace kctsb::internal

// ============================================================================
// C ABI Export (extern "C")
// ============================================================================
extern "C" {

kctsb_error_t kctsb_algorithm_init(kctsb_algorithm_ctx_t* ctx,
                                    const uint8_t* key, size_t key_len) {
    if (!ctx || !key) {
        return KCTSB_ERROR_INVALID_PARAM;
    }

    try {
        // Use placement new to construct C++ object in C struct
        new (&ctx->internal) kctsb::internal::Algorithm();
        // Initialize...
        return KCTSB_SUCCESS;
    } catch (...) {
        return KCTSB_ERROR_INTERNAL;
    }
}

void kctsb_algorithm_clear(kctsb_algorithm_ctx_t* ctx) {
    if (ctx) {
        // Secure zero memory
        volatile uint8_t* p = reinterpret_cast<volatile uint8_t*>(ctx);
        for (size_t i = 0; i < sizeof(*ctx); ++i) {
            p[i] = 0;
        }
    }
}

} // extern "C"
```

### 头文件模板

```c
/**
 * @file algorithm.h
 * @brief Algorithm Public API
 */

#ifndef KCTSB_CRYPTO_ALGORITHM_H
#define KCTSB_CRYPTO_ALGORITHM_H

#include "kctsb/core/common.h"

#ifdef __cplusplus
extern "C" {
#endif

// Constants
#define KCTSB_ALGORITHM_BLOCK_SIZE 16
#define KCTSB_ALGORITHM_KEY_SIZE 32

// Context structure
typedef struct kctsb_algorithm_ctx_s {
    uint8_t opaque[256];  // Opaque storage for C++ implementation
} kctsb_algorithm_ctx_t;

// C API Functions
KCTSB_API kctsb_error_t kctsb_algorithm_init(kctsb_algorithm_ctx_t* ctx,
                                              const uint8_t* key, size_t key_len);

KCTSB_API kctsb_error_t kctsb_algorithm_process(const kctsb_algorithm_ctx_t* ctx,
                                                  const uint8_t* input, size_t input_len,
                                                  uint8_t* output);

KCTSB_API void kctsb_algorithm_clear(kctsb_algorithm_ctx_t* ctx);

#ifdef __cplusplus
}
#endif

#endif // KCTSB_CRYPTO_ALGORITHM_H
```

---

## 🔧 编译优化配置

### CMake 优化参数

```cmake
# C++17 强制标准
set(CMAKE_CXX_STANDARD 17)
set(CMAKE_CXX_STANDARD_REQUIRED ON)
set(CMAKE_CXX_EXTENSIONS OFF)

# Release 极限优化
if(CMAKE_BUILD_TYPE STREQUAL "Release")
    if(CMAKE_CXX_COMPILER_ID MATCHES "GNU|Clang")
        add_compile_options(
            # 基础优化
            -O3
            -march=native
            -mtune=native
            -ffast-math
            -funroll-loops
            -fomit-frame-pointer
            -flto
            -fPIC

            # 内存优化
            -fno-rtti              # 禁用 RTTI，减少内存占用
            -fno-exceptions        # 禁用异常（C ABI 层处理）

            # 硬件加速
            -maes                  # AES-NI
            -mpclmul               # PCLMUL (GCM)
            -msse4.1 -msse4.2      # SSE4
            -mavx2                 # AVX2

            # 安全加固
            -fstack-protector-strong
            -D_FORTIFY_SOURCE=2
        )
        add_link_options(-flto)
    endif()
endif()
```

### build.sh 优化

```bash
# 默认极限优化参数
CMAKE_BUILD_TYPE="Release"
OPTIMIZATION_FLAGS="-O3 -march=native -mtune=native -ffast-math -funroll-loops -fomit-frame-pointer -flto -fno-rtti -fno-exceptions"

# 硬件加速标志
SIMD_FLAGS="-maes -mpclmul -msse4.1 -msse4.2 -mavx2"

# 完整编译命令
cmake -B build -G Ninja \
    -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_CXX_FLAGS="$OPTIMIZATION_FLAGS $SIMD_FLAGS" \
    -DCMAKE_C_FLAGS="-O3 -march=native -flto" \
    -DKCTSB_BUILD_TESTS=ON \
    -DKCTSB_BUILD_BENCHMARKS=ON
```

---

## ✅ 实施清单

### Phase 1: 基础设施 (Day 1)

- [ ] 更新 AGENTS.md 开发规范（三原则）
- [ ] 更新 CMakeLists.txt 编译配置
- [ ] 更新 scripts/build.sh 脚本
- [ ] 更新 version.h 到 3.4.0

### Phase 2: Hash 模块重构 (Day 1-2)

- [ ] 重构 sha256.cpp + sha256.h
- [ ] 重构 sha512.cpp + sha512.h
- [ ] 重构 sha3.cpp + sha3.h (合并 Keccak)
- [ ] 重构 blake2.cpp + blake2.h
- [ ] 新增 blake3.cpp + blake3.h
- [ ] 重构 hmac.cpp + hmac.h
- [ ] 删除冗余文件
- [ ] 完善 test_hash.cpp 单元测试
- [ ] 更新 benchmark_hash.cpp 覆盖全部 hash

### Phase 3: SM 系列重构 (Day 2-3)

- [ ] 重构 sm3.cpp + sm3.h
- [ ] 重构 sm4.cpp + sm4.h
- [ ] 重构 sm2.cpp + sm2.h
- [ ] 删除 sm_api.cpp 及相关冗余文件
- [ ] 更新 test_sm.cpp

### Phase 4: 对称加密重构 (Day 3)

- [ ] 重构 aes.cpp + aes.h
- [ ] 重构 chacha20.cpp + chacha20.h
- [ ] 更新 test_aes.cpp

### Phase 5: 非对称加密重构 (Day 4)

- [ ] 重构 rsa.cpp + rsa.h
- [ ] 重构 ecc.cpp + ecc.h (合并 ecdh/ecdsa/ecies)
- [ ] 更新 test_rsa.cpp, test_ecc.cpp

### Phase 6: 高级密码学重构 (Day 4-5)

- [ ] 重构白盒密码模块
- [ ] 重构秘密共享模块
- [ ] 重构零知识证明模块
- [ ] 重构后量子密码模块

### Phase 7: 测试与发布 (Day 5)

- [ ] 运行全量单元测试
- [ ] 运行全量集成测试
- [ ] 运行全量 benchmark
- [ ] 创建 v3.4.0-release.md
- [ ] 更新 README.md
- [ ] Git 提交发布

---

## 📊 验收标准

1. **编译验证**
   - [ ] macOS: GCC/Clang 编译零警告
   - [ ] Linux: GCC 编译零警告
   - [ ] Windows: MinGW/MSVC 编译零警告

2. **测试验证**
   - [ ] 全部单元测试通过 (目标: 100+)
   - [ ] 全部集成测试通过
   - [ ] Benchmark 性能不低于 v3.3.3

3. **代码质量**
   - [ ] 每个算法: 1 .cpp + 1 .h
   - [ ] 无额外 API 封装文件
   - [ ] C ABI 在各算法 .cpp 文件内导出

4. **文档完整**
   - [ ] AGENTS.md 更新
   - [ ] README.md 更新
   - [ ] v3.4.0-release.md 发布说明

---

## 🔗 参考资源

- [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/)
- [NIST FIPS 202 (SHA-3)](https://csrc.nist.gov/publications/detail/fips/202/final)
- [RFC 7693 (BLAKE2)](https://tools.ietf.org/html/rfc7693)
- [GM/T 0003-2012 (SM2)](http://www.gmbz.org.cn/)
