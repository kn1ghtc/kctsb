...
> **状态**: ✅ NTT 核心已完成 | ✅ AVX2 已完成 | ✅ RNS 已完成 | ✅ NTT PolyOps 已完成

---

## 0. 实施进度 (2026-01-22 更新)
...
| 组件 | 优先级 | 预计时间 | 备注 |
|------|--------|----------|------|
| BGV 完整 RNS-CRT 集成 | 高 | 4h | 需要 CRT 重构回 ZZ 模数 |
| 模数切换 | 中 | 2h | rescale 操作 |
...
新增/修改文件:
  ...
  include/kctsb/advanced/fe/bgv/bgv_ntt_helper.hpp  # BGV-NTT 桥接层
  ...
  src/advanced/fe/bgv/bgv_evaluator.cpp             # 集成 NTT helper (待完整 RNS-CRT)
...
RNS 特性:
  - RNSBase: 多模数基类, CRT 常量预计算
  - RNSPoly: 多级多项式表示, NTT 形式转换
  - RNSBaseConverter: BEHZ 风格快速基转换
  - generate_ntt_primes: 自动生成 NTT-friendly 素数

AVX2 特性:
  - add_mod_avx2: 4x parallel 模加法 (256-bit)
  - sub_mod_avx2: 4x parallel 模减法 (256-bit)
  - forward_avx2 / inverse_avx2: 向量化 NTT 蝴蝶操作
  - forward_negacyclic_avx2 / inverse_negacyclic_avx2: 向量化负循环 NTT
  - poly_multiply_negacyclic_ntt_avx2: 完整 AVX2 加速多项式乘法

NTT PolyOps 特性:
  - multiply_poly_ntt: O(n log n) 负循环乘法
  - multiply_poly_ntt_rns: RNS 批量乘法
  - add_poly_mod / sub_poly_mod: 向量化算术
  - to_ntt_form / from_ntt_form: 形式转换
  - negate_poly_mod / scalar_mul_poly_mod: 辅助操作

BGV NTT Helper 特性:
  - can_use_ntt: 检查参数是否支持 NTT 加速
  - zz_px_to_uint64 / uint64_to_zz_px: 类型转换
  - multiply_ntt: 单质数 NTT 乘法
  - multiply_rns_ntt: 多质数 RNS-NTT 乘法 (待 CRT 重构)
```
